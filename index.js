...
function filterUnfundedOnly() {
    ...
    const notFunded = GAMES_JSON.filter(game => game.pledged < game.goal);
...
const numUnFunded = GAMES_JSON.reduce((acc, game) => acc + (game.pledged < game.goal), 0);



// create a string that explains the number of unfunded games using the ternary operator
const unFundedText = `
    $${totalRaised.toLocaleString()} raised for ${GAMES_JSON.length} games.
    ${numUnFunded === 1 ? '1 game remains unfunded' : `${numUnFunded} games are still unfunded.`}
`;
